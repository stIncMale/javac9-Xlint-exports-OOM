...
```javac 9``` (```Java HotSpot(TM) 64-Bit Server VM (build 9+181, mixed mode)``` on ```masOS Sierra 10.12.6 (16G29)```)
...
```
It also goes away if ```-Xlint:exports``` is enabled, but the completely unnecessary ```guava-23.3-jre.jar``` is removed from ```--module-path```:
```
javac -J-Xmx100M -Xlint:exports,-requires-automatic,-requires-transitive-automatic --module-path /Users/male/.m2/repository/com/datadoghq/java-dogstatsd-client/2.3/java-dogstatsd-client-2.3.jar:/Users/male/.m2/repository/com/google/code/findbugs/jsr305/3.0.2/jsr305-3.0.2.jar ./src/main/java/module-info.java ./src/main/java/mypackage/MyClass.java
```

Option ```-Xlint:exports``` is very important, because it tells you when you need to specify ```requires transitive```, which is virtually
